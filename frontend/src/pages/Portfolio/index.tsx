...
interface PortfolioItem {
  ...
  description: string;
  images: string[];
  category: string; // Deve ser "Modelos" no Firebase
  demoUrl?: string; // Link para ver ao vivo
}
...
export const Portfolio = () => {
  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      try {
        // Busca apenas produtos da categoria "Modelos"
        const q = query(collection(db, 'products'), where('category', '==', 'Modelos'));
      ...
      <div className="container mx-auto px-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-gray-800/50 rounded-3xl border border-gray-700">
            <p className="text-gray-400 text-lg">Em breve adicionaremos nossos modelos aqui!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {projects.map((project) => (
              <div key={project.id} className="group flex flex-col bg-gray-800 rounded-3xl overflow-hidden border border-gray-700 hover:border-gray-500 transition-all duration-300 hover:shadow-2xl hover:shadow-primary/10">
                
                {/* Janela do Navegador (Visual) */}
                <div className="relative aspect-[16/10] bg-gray-900 overflow-hidden">
                  ...
                  <img 
                    ...
                    alt={project.name} 
                    className="w-full h-full object-cover pt-8 transition-transform duration-700 group-hover:scale-105 group-hover:rotate-1"
                  />

                  {/* Overlay com Ações */}
                  <div className="absolute inset-0 bg-gray-900/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-4 p-6 text-center">
                    ...
                    <div className="flex gap-3 translate-y-4 group-hover:translate-y-0 transition-transform duration-300 delay-100">
                      {project.demoUrl && (
                        <a 
                          ...
                          rel="noreferrer"
                        >
                          <Button className="bg-white text-gray-900 hover:bg-gray-200 border-none font-bold px-6">
                            Ver Online
                          </Button>
                        </a>
                      )}
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="mt-auto">
                    <Link to={`/produtos/${project.id}`} className="block">
                      <button className="w-full py-3 rounded-xl border border-gray-600 text-gray-300 hover:bg-white hover:text-gray-900 hover:border-white transition-all font-medium flex items-center justify-center gap-2">
